const n=`{\r
 "cells": [\r
  ...
  {\r
   ...
   "metadata": {},\r
   "source": [\r
    "接下来，我们将动手编写实现基于视觉词袋模型的图像分类算法。我们先导入必要的库，并导入Caltech-101数据集。"\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T19:00:39.189642Z",\r
     "iopub.status.busy": "2024-12-03T19:00:39.189303Z",\r
     "iopub.status.idle": "2024-12-03T19:00:49.344519Z",\r
     "shell.execute_reply": "2024-12-03T19:00:49.342657Z",\r
     "shell.execute_reply.started": "2024-12-03T19:00:39.189607Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [],\r
  ...
  {\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T19:00:56.750681Z",\r
     "iopub.status.busy": "2024-12-03T19:00:56.750216Z",\r
     "iopub.status.idle": "2024-12-03T19:01:32.317113Z",\r
     "shell.execute_reply": "2024-12-03T19:01:32.315884Z",\r
     "shell.execute_reply.started": "2024-12-03T19:00:56.750637Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [],\r
  ...
  {\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T19:01:43.294246Z",\r
     "iopub.status.busy": "2024-12-03T19:01:43.293809Z",\r
     "iopub.status.idle": "2024-12-03T19:01:43.898497Z",\r
     "shell.execute_reply": "2024-12-03T19:01:43.897052Z",\r
     "shell.execute_reply.started": "2024-12-03T19:01:43.294212Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [\r
  ...
  {\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2022-11-14T04:15:42.707808Z",\r
     "iopub.status.busy": "2022-11-14T04:15:42.706668Z",\r
     "iopub.status.idle": "2022-11-14T04:15:42.717775Z",\r
     "shell.execute_reply": "2022-11-14T04:15:42.716426Z",\r
     "shell.execute_reply.started": "2022-11-14T04:15:42.707764Z"\r
    }\r
   },\r
   "source": [\r
    "我们先进行视觉词典的构建。为了得到视觉词典，首先需要对输入图像的表征进行提取，这里使用SIFT对图像进行处理。"\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T19:01:46.199836Z",\r
     "iopub.status.busy": "2024-12-03T19:01:46.199431Z",\r
     "iopub.status.idle": "2024-12-03T19:03:15.830468Z",\r
     "shell.execute_reply": "2024-12-03T19:03:15.829162Z",\r
     "shell.execute_reply.started": "2024-12-03T19:01:46.199800Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [],\r
   "source": [\r
    ...
    "        vec_dict[y_train[i]]['des'][kp_vector[k]] = des_vector[k]"\r
   ]\r
  },\r
  {\r
   "cell_type": "markdown",\r
   "metadata": {},\r
   "source": [\r
    "接着，为了保持类别之间的平衡，为每个类别选取相同个数的特征用于之后的聚类。先统计每个类别的SIFT图像特征个数，找到拥有最少特征个数的类别，然后以该类别的特征个数为标准个数，从每个类别的特征中筛选出标准个数的特征进行后续的聚类任务。"\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T19:06:49.636848Z",\r
     "iopub.status.busy": "2024-12-03T19:06:49.635984Z",\r
     "iopub.status.idle": "2024-12-03T19:06:50.591340Z",\r
     "shell.execute_reply": "2024-12-03T19:06:50.590130Z",\r
     "shell.execute_reply.started": "2024-12-03T19:06:49.636798Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [],\r
  ...
  {\r
   ...
   "metadata": {},\r
   "source": [\r
    "然后，使用$k$-means聚类算法将这些特征进行聚类，得到的聚类中心组成视觉词典。这里，将聚类中心个数设置为200。"\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T19:07:41.340947Z",\r
     "iopub.status.busy": "2024-12-03T19:07:41.340507Z",\r
     "iopub.status.idle": "2024-12-03T19:21:49.036470Z",\r
     "shell.execute_reply": "2024-12-03T19:21:49.035082Z",\r
     "shell.execute_reply.started": "2024-12-03T19:07:41.340911Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [],\r
   "source": [\r
    ...
    "N_clusters = 200\\n",\r
    "kmeans = KMeans(n_clusters=N_clusters, random_state=0).fit(vec_list)"\r
   ]\r
  },\r
  {\r
   "cell_type": "markdown",\r
  ...
  {\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T19:22:15.145246Z",\r
     "iopub.status.busy": "2024-12-03T19:22:15.144834Z",\r
     "iopub.status.idle": "2024-12-03T19:22:15.158571Z",\r
     "shell.execute_reply": "2024-12-03T19:22:15.157175Z",\r
     "shell.execute_reply.started": "2024-12-03T19:22:15.145213Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [],\r
  ...
  {\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T19:22:23.283823Z",\r
     "iopub.status.busy": "2024-12-03T19:22:23.283424Z",\r
     "iopub.status.idle": "2024-12-03T19:52:27.955380Z",\r
     "shell.execute_reply": "2024-12-03T19:52:27.953739Z",\r
     "shell.execute_reply.started": "2024-12-03T19:22:23.283791Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [],\r
  ...
  {\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T19:53:01.893970Z",\r
     "iopub.status.busy": "2024-12-03T19:53:01.893469Z",\r
     "iopub.status.idle": "2024-12-03T20:02:59.354768Z",\r
     "shell.execute_reply": "2024-12-03T20:02:59.353272Z",\r
     "shell.execute_reply.started": "2024-12-03T19:53:01.893926Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [],\r
  ...
  {\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T20:08:38.781170Z",\r
     "iopub.status.busy": "2024-12-03T20:08:38.780723Z",\r
     "iopub.status.idle": "2024-12-03T20:09:56.283098Z",\r
     "shell.execute_reply": "2024-12-03T20:09:56.281891Z",\r
     "shell.execute_reply.started": "2024-12-03T20:08:38.781118Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [\r
  ...
  {\r
   ...
   "metadata": {},\r
   "source": [\r
    "最后，在测试集上对分类器进行评估，并计算 top-1 错误率。"\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
   ...
   "metadata": {\r
    "execution": {\r
     "iopub.execute_input": "2024-12-03T20:10:21.193007Z",\r
     "iopub.status.busy": "2024-12-03T20:10:21.192558Z",\r
     "iopub.status.idle": "2024-12-03T20:10:21.845889Z",\r
     "shell.execute_reply": "2024-12-03T20:10:21.844161Z",\r
     "shell.execute_reply.started": "2024-12-03T20:10:21.192974Z"\r
    },\r
    "trusted": true\r
   },\r
   "outputs": [\r
   ...
   "source": [\r
    ...
    "print('top-1 error: ', tep/len(y_test))"\r
   ]\r
  },\r
  {\r
   "cell_type": "markdown",\r
  ...
  {\r
   ...
   "metadata": {},\r
   "source": [\r
    "可以使用nn.Covn2d()函数实现卷积操作，该函数定义如下："\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
  ...
  {\r
   ...
   "metadata": {},\r
   "source": [\r
    "接下来，我们将动手编写完成基于Resnet34的图像分类任务。首先，先导入必要的库。"\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
  ...
  {\r
   ...
   "metadata": {},\r
   "source": [\r
    "接着，把Caltech-101数据集封装成类，并生成训练集类、验证集类以及测试集类。"\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
   ...
   "source": [\r
    ...
    "test_data = ImageDataset(x_test, y_test, val_transform)"\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
   "execution_count": 16,\r
   "metadata": {\r
    "trusted": true\r
   },\r
   "outputs": [],\r
   "source": [\r
    ...
    "testloader = DataLoader(test_data, batch_size=BATCH_SIZE, shuffle=False)"\r
   ]\r
  },\r
  {\r
   "cell_type": "markdown",\r
   "metadata": {},\r
   "source": [\r
    "接下来，我们开始加载ResNet34模型。这里将直接调用已经封装好的库。"\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
  ...
  {\r
   ...
   "source": [\r
    "\\n",\r
    "继续定义训练时需要用到的优化器和损失函数："\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
   "execution_count": 18,\r
   "metadata": {\r
    "trusted": true\r
   },\r
   "outputs": [],\r
   "source": [\r
    ...
    "# 定义损失函数\\n",\r
    "criterion = nn.CrossEntropyLoss()"\r
   ]\r
  },\r
  {\r
   "cell_type": "markdown",\r
   "metadata": {},\r
   "source": [\r
    "接下来，编写训练模型、验证模型以及测试模型top-1错误率的函数。"\r
   ]\r
  },\r
  {\r
   "cell_type": "code",\r
  ...
  {\r
   ...
   "outputs": [\r
    {\r
     "data": {\r
      "text/plain": [\r
       "<matplotlib.legend.Legend at 0x7fd1fd5df880>"\r
      ]\r
     },\r
     "execution_count": 30,\r
     "metadata": {},\r
     "output_type": "execute_result"\r
    },\r
    {\r
     "data": {\r
      ...
      "text/plain": [\r
       "<Figure size 1000x700 with 1 Axes>"\r
      ]\r
     },\r
     "metadata": {},\r
 ...
 "metadata": {\r
  "kaggle": {\r
   ...
   "dockerImageVersionId": 30301,\r
   "isGpuEnabled": false,\r
   "isInternetEnabled": true,\r
   "language": "python",\r
   "sourceType": "notebook"\r
  },\r
  "kernelspec": {\r
  ...
  "vscode": {\r
   "interpreter": {\r
    "hash": "aee8b7b246df8f9039afb4144a1f6fd8d2ca17a180786b69acc140d282b71a49"\r
   }\r
  }\r
 },\r
 "nbformat": 4,\r
