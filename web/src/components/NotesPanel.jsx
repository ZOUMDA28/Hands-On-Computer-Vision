...
export default function NotesPanel({
  catalog, bookmarks, notes, notebooksWithNotes, getSectionNotes, exportData, importFile, onClearAll, onSelect,
}) {
  ...
  const handleImport = async (e) => {
    ...
    if (result.ok) {
      alert(`导入成功：${result.bookmarkCount} 个收藏，${result.noteCount} 条笔记`)
    } else {
  ...
  return (
    <div className="viewer">
      <div className="viewer-header visible">
        <h1 className="viewer-title">笔记与收藏</h1>
        ...
        <div className="notes-panel">
          ...
          <section className="notes-section">
            ...
            {bookmarkList.length === 0 ? (
              <p className="notes-empty">还没有收藏任何 Notebook。阅读时点击标题旁的星标即可收藏。</p>
            ) : (
          ...
          <section className="notes-section">
            ...
            {allNotes.length === 0 ? (
              <p className="notes-empty">还没有笔记。阅读时选中文字，在弹出工具栏中选择"笔记"或"高亮"即可添加。</p>
            ) : (
                        ...
                        <div key={n.id}
                          ...
                          <div className="notes-card-meta">
                            <span className="notes-card-date">
                              {new Date(n.updatedAt || Date.now()).toLocaleDateString('zh-CN')}
                            </span>
